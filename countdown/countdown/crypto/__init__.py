...
from countdown.crypto import kem_registry, sigalg_registry
from countdown.crypto.kem_registry import KemGroup
...
from countdown.crypto.tls_parser import Handshake, parse_tls_handshake
...
def parse_handshake(flow: "Flow") -> Handshake:
    ...
    if _is_quic(flow, client, server):
        from countdown.crypto.quic_parser import parse_quic_handshake  # lazy: heavy crypto dep
...
def _is_quic(flow: "Flow", client: bytes, server: bytes) -> bool:
    ...
    if flow.l4_proto != "udp":
        return False
    from countdown.flows.handshake import quic_long_header
