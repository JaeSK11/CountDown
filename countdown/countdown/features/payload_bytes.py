...
* :class:`PayloadBytes` -- the extractor that turns *our* :class:`~countdown.schema.Flow`
  objects into the same token matrix, so the byte member can eventually train on
  countdown-parsed pcaps rather than only on ET-BERT's shipped arrays.
...
``countdown.data.etbert_corpus``) so that no tokenisation difference can be confused with
