...
- **Canonical class ordering (`LabelSpace`) — resolved, A2.** Models bind to a `LabelSpace`, *not*
  ...
  `LabelSpace` is a new frozen dataclass in `countdown/schema.py`, where the taxonomy already lives:
...
```python
from countdown.data import load_pooled
...
add to `FlowDataset` in `countdown/schema.py`.
...
**Resolution:** new frozen dataclass in `countdown/schema.py`; full spec in §5.1. The load-bearing
...
`countdown/data/pooled.py` exposing `load_pooled(names, target=..., **kw) -> FlowDataset`, plus a
