...
from countdown.eval import evaluate, group_split, split_report
from countdown.models import ModelRegistry, load_model
from countdown.schema import FiveTuple, Flow, FlowDataset, LabelSpace
from countdown.training import ExperimentConfig, train
...
def test_training_harness_writes_a_self_describing_run(tmp_path, monkeypatch):
    ds = _synthetic_dataset()
    monkeypatch.setattr("countdown.data.load", lambda *a, **k: ds, raising=False)
    monkeypatch.setattr("countdown.data.base.load", lambda *a, **k: ds)
...
def test_harness_rejects_a_feature_model_mismatch(monkeypatch):
    ds = _synthetic_dataset()
    # train() resolves `from countdown.data import load`, so the re-exported name is the
