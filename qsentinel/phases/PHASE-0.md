...
**One-line exit criterion:** `qsentinel.data.load("iscxvpn", target="traffic_type")` returns a
...
**DoD:** `pip install -e .` succeeds; `python -c "import qsentinel"` works.
...
| # | Task | DoD |
| --- | --- | --- |
| 0.1 | Repo scaffold + `pyproject.toml` + `data` symlink + logging | `pip install -e .`; `import qsentinel` |
