...
└── data/
    ...
    └── sig_schemes.yaml
scripts/
├── validate_pqc.py      # qsentinel vs tshark; precision/recall report
...
1. **Oracle cross-check:** run `tshark -T fields -e tls.handshake.extensions_key_share_group` on a
   sample of CSTNET/PostQuantumTLS/self-gen pcaps; assert qsentinel's `negotiated_group` matches on
...
# oracle agreement
# validate_pqc.py: qsentinel negotiated_group == tshark group on >=99% of parseable flows
```
