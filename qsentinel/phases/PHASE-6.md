...
**One-line exit criterion:** `qsentinel analyze <pcap>` emits per-flow verdicts
`{five_tuple, l7, tls_version, cipher, kem_group, pqc_verdict, traffic_class?, confidence}`, and
`qsentinel experiments run --all` regenerates every results table (PQC-triage %, per-target
...
- `cli.py` — `qsentinel {analyze, live, train, eval, experiments}` console entry.
...
├── cli.py                  # qsentinel {analyze,live,train,eval,experiments}
...
### 4.2 CLI (`cli.py`)
- `qsentinel analyze <pcap> [--out csv|json] [--mode full|fast]`
- `qsentinel live <iface> [--bpf ...] [--mode fast]`
- `qsentinel train|eval` — unify P3–P5 training/eval behind one entry.
- `qsentinel experiments run [--all|--name X]` — regenerate results.
- **DoD:** `qsentinel analyze` on an example pcap prints/writes verdicts.
...
$ qsentinel analyze examples/cstnet_sample.pcap --out verdicts.csv
...
$ qsentinel experiments run --all      # -> results/{pqc_triage,per_target,ablation,cross_dataset}.*

# 4) live
$ sudo qsentinel live eth0 --bpf "tcp port 443" --mode fast   # streams verdicts
```
